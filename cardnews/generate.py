...
def card_08():
    ...
    d.text((MARGIN, y), "포켓몬고 데이터가", font=F('ko_eb', 64), fill=INK)
    y += 78
    d.text((MARGIN, y), "LA 인도를 달린다", font=F('ko_eb', 64), fill=INK)
    y += 86

    points = [
        ("→", "Coco 로봇 4대 카메라가 주행 중 360° 주변을 실시간으로 촬영한다"),
        ("→", "나이언틱 VPS¹가 그 영상과 3D 지도를 비교해 로봇 위치를 cm 단위로 계산한다"),
        ("→", "이 3D 지도의 기반 — 포켓몬고 플레이어들이 수년간 LA 거리를 스캔한 Lightship² 데이터다"),
        ("→", "GPS 신호가 흔들리는 빌딩 숲에서도 로봇은 인도 폭 안에서 벗어나지 않는다"),
    ]
    ...
    y += 10
    b_end = "LA 어딘가에서 포켓스탑을 스캔한 누군가는 알지 못했을 것이다. 자신이 배달 로봇의 눈을 만들고 있었다는 사실을. 그것이 플랫폼이 데이터를 인프라로 바꾸는 방식이다."
    multiline(d, b_end, F('ko_sb', 28), MARGIN, y, W - MARGIN*2, lhf=1.55)

    footnote_zone(d, [
        ("¹", "VPS (Visual Positioning System)",
         "카메라 이미지와 사전 구축된 3D 공간 지도를 비교해 위치를 계산하는 기술. GPS와 달리 위성 신호 없이 작동하며, 실내·지하에서도 cm 단위 정확도를 달성한다."),
        ("²", "Niantic Lightship",
         "나이언틱의 AR 개발 플랫폼. 포켓몬고 이용자들이 포켓스탑을 스캔할 때 수집된 3D 공간 데이터가 Lightship의 VPS 지도 데이터베이스를 구성한다."),
    ], H - 300)
...
def card_09():
    ...
    b1 = "나이언틱은 게임 회사로 시작해 AI 인프라 회사가 됐다. 전환점은 제품이 아니라 데이터였다. 게임 플레이 하나하나가 현실 세계를 이해하는 AI의 학습 재료가 됐고, 그 AI가 지금 산업 현장에서 작동하고 있다. 이것이 밸류체인¹이다."
    y = multiline(d, b1, F('ko', 29), MARGIN, y, W - MARGIN*2, lhf=1.56)
    y += 20

    b2 = "데이터 해자²는 이렇게 만들어진다. 경쟁사가 따라오기 시작한 시점에 이미 격차는 10년이다. 그리고 그 격차는 오늘도 넓어지고 있다. 이 흐름을 먼저 읽는 사람이 다음 번 나이언틱을 먼저 본다."
...
def card_10():
    ...
    d.text((MARGIN, y), "이 패턴, 다음엔", font=F('ko_eb', 60), fill=INK)
    y += 74
    d.text((MARGIN, y), "어디서 보일까", font=F('ko_eb', 60), fill=INK)
    y += 86

    points = [
        "포켓몬고는 게임이 아니었다 — 수억 명을 고용하지 않고 현실 세계를 스캔하는 분산 수집 엔진이었다",
        "300억 장의 이미지는 GPS를 대체하는 위치 기술을 만들었다. 데이터가 기술을 기다렸다",
        "Coco Robotics 협업은 '게임 → 데이터 → AI → 로보틱스'의 밸류체인이 실제로 작동함을 증명했다",
        "데이터 해자의 깊이는 경쟁사가 따라오기 시작한 시점에 이미 결정된다",
        "플랫폼이 만드는 데이터의 향방을 먼저 읽는 것 — 그것이 f00tnotes가 추적하는 것이다",
    ]
    ...
    y += 36
    d.text((MARGIN, y), "공간 AI의 다음 전장", font=F('ko_sb', 28), fill=INK)
